...
console.warn(
  "Using editor.js compatibility layer - consider updating your imports to use the modular structure directly"
);
...
function createBlockElement(type, content = "") {
  ...
  blockContainer.className =
    "block-container group relative transition-all duration-200 hover:bg-surface-50 rounded-lg p-3";
  ...
  switch (type) {
    ...
    case "list":
      ...
      blockElement.className =
        "editable-block list-block pl-5 list-disc space-y-1";
    ...
    case "database":
      ...
      dbWrapper.className =
        "border border-surface-200 rounded-lg p-4 bg-surface-50";
    ...
    default: // Default to text block
      ...
      // Apply base editable and specific text styles
      blockElement.className =
        "editable-block text-block text-surface-800 leading-relaxed";
      blockElement.contentEditable = true;
      blockElement.dataset.placeholder =
...
function handleDragStart(e) {
  ...
  if (!dropIndicator) {
    ...
    dropIndicator.className =
      "drag-indicator h-1 bg-primary-500 rounded-full shadow-sm";
