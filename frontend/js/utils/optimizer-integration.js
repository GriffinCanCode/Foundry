      ...
      if (targetItem && targetItem !== draggedItem) {
          ...
          if (lastBlock && e.clientY > lastBlock.getBoundingClientRect().bottom) {
              ...
              if (this.dropIndicator) {
                 ...
                 this.dropIndicator.style.top = `${lastBlock.getBoundingClientRect().bottom + window.scrollY - 2}px`;
                 this.dropIndicator.style.left = `${editorRect.left + window.scrollX + 10}px`;
                 ...
                 this.dropIndicator.style.transition = 'opacity 150ms, transform 150ms';
             }
          } else {
             this.hideDropIndicator();
          }
...
function loadNonCriticalEditorFeatures() {
  ...
  import('../modules/editor-extensions.js').then(module => {
    module.initializeExtensions();
  }).catch(error => {
    console.warn('Failed to load editor extensions (this is expected if the module doesn\'t exist yet)', error);
  });
}
