...
const newPageHandler = () => createNewDocument();
const manageWorkplacesHandler = () => {
    console.log('Manage workplaces button clicked via delegation');
    showWorkspaceSelection();
};
const workspaceSwitcherHandler = (e) => {
    // Prevent default if it's an anchor
    if (e && e.preventDefault) e.preventDefault();
    
    // Show visual feedback on the button when clicked
    const button = document.getElementById('workspace-switcher');
    if (button) {
        // Add a temporary active class for visual feedback
    ...
    // Call workspace selection with error handling
    try {
        showWorkspaceSelection();
    } catch (error) {
        console.error('Error showing workspace selection:', error);
        showNotification('Failed to load workspace selection', 'error');
    }
};
...
export function setupSidebarButtons() {
    ...
    delegateEvent(sidebarEl, 'click', '#manage-workplaces-btn', manageWorkplacesHandler);
    delegateEvent(sidebarEl, 'click', '#workspace-switcher', workspaceSwitcherHandler);
